...
(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["js-infinite-scroller"] = factory();
	else
		root["js-infinite-scroller"] = factory();
 ...
 */var InfiniteScroller=function(){function InfiniteScroller(scroller,source,config){_classCallCheck(this,InfiniteScroller);if(!scroller||!source){throw new Error('缺乏必要参数');}config=config||{};this.listMarginTop=config.listMarginTop||this.getOffsetTop(scroller);this.runwayItems=config.runwayItems||10;this.runwayItemsOpposite=config.runwayItemsOpposite||10;this.collectBottomDOMFlag=config.collectBottomDOMFlag||true;// TODO： 如果需要回收刚加载完成的数据 此处使用有问题，待修复
...
if(this._requestInProgress)return;var itemsNeeded=this.lastAttachedItemIndex-this._loadedItems;if(itemsNeeded<=0)return;this._requestInProgress=true;this._source.fetch(itemsNeeded).then(function(item){if(item.length){_this._addContent(item);}});}/**
   ...
   *     scroller list.
   */},{key:'_addContent',value:function _addContent(items){this._requestInProgress=false;this.collectBottomDOMFlag=false;for(var i=0;i<items.length;i++){if(this._items.length<=this._loadedItems)this._addItem();var thisIndex=this._loadedItems;this._items[thisIndex].data=items[i];items[i].__InfiniteScrollerIndex=thisIndex;this._loadedItems++;}this._attachContent('fetch');}},{key:'_updateContentPos',value:function _updateContentPos(begin,end,delta){for(var i=begin;i<end;i++){this._items[i].top=this._items[i].top+delta;if(this._items[i].node){this._items[i].node.style.transform='translateY('+this._items[i].top+'px)';}}}},{key:'getOffsetTop',value:function getOffsetTop(elements){var top=elements.offsetTop;var parent=elements.offsetParent;while(parent!=null){top+=parent.offsetTop;parent=parent.offsetParent;};return top;}/**
   * 更新列表距离顶部的高度
   * @param {number} listMarginTop 非必须，更新后的列表距离顶端的高度，不传则直接计算
   */},{key:'resizeList',value:function resizeList(listMarginTop){listMarginTop=parseInt(listMarginTop,10);if(listMarginTop===listMarginTop){this.listMarginTop=listMarginTop;}else{this.listMarginTop=this.getOffsetTop(this._scroller);}}/**
   * 列表中某一项高度变化
